...
// ── AC-E3: 복합 위협 보너스 (개선 전 실패 예상) ──────────────────────────────
describe('evaluateBoard — 복합 위협 보너스 (개선 전 실패 예상)', () => {
  it('열린3+열린3 교차: 단순 합산(1000)보다 3000+ 높아야 한다', () => {
    const board = emptyBoard();
    // 가로 열린3: (7,5)(7,6)(7,7)
    board[7][5] = board[7][6] = board[7][7] = 'B';
    // 세로 열린3: (5,7)(6,7) + (7,7) 공유
    board[5][7] = board[6][7] = 'B';
    const score = evaluateBoard(board, 'B');
    // 복합 보너스 없으면: 각 돌 단순 합산 ~3000
    // 복합 보너스 있으면: 교차 돌(7,7)에 +3000 추가 → ~6000+
    expect(score).toBeGreaterThan(5000);
  });

  it('닫힌4+열린3 교차: 단순 합산(1500)보다 4000+ 높아야 한다', () => {
    const board = emptyBoard();
    // 가로 닫힌4: B(7,4)(7,5)(7,6)(7,7), 왼쪽 W로 막힘
    board[7][4] = board[7][5] = board[7][6] = board[7][7] = 'B';
    board[7][3] = 'W'; // 왼쪽 막힘 → 닫힌4
    // 세로 열린3: (5,7)(6,7)(7,7) 공유
    board[5][7] = board[6][7] = 'B';
    const score = evaluateBoard(board, 'B');
    expect(score).toBeGreaterThan(5000);
