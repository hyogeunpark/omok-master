...
각 난이도의 탐색 방식을 사용자에게 한 줄로 노출한다. 문구는 `src/ui/brainLabel.js`의 `BRAIN_LABEL` 하나에서 관리하며 아래 두 곳에서 **동일하게** 표시한다.
...
- **난이도 선택 카드** (`StartScreen`) — 카드마다 해당 문구를 골드 pill(`.difficulty-brain`)로 표시.
- **게임 화면** (`Game`) — 헤더의 "CPU {색}" 아래에 `CPU 두뇌` 캡션과 함께 같은 pill로 표시(`.game-cpu-brain`).
...
**문제**: `minimaxMove`/`minimaxMoveTT`는 결정적이라 같은 판이면 항상 같은 수를 둔다 → 같은 수순 → 같은 모양 반복(피드백). 쉬움만 랜덤이 있고 보통·어려움은 반복적이다.

**규칙**: 최선점(bestScore)과의 차이가 **margin 이내인 수들 중에서 랜덤**으로 하나를 고른다.
- `margin=0`이면 기존과 동일(결정적). `margin>0`이면 `score ≥ bestScore − margin`인 후보 풀에서 균등 랜덤.
- **강함 보존이 핵심**: margin을 **작게** 잡아 "사실상 동급인 수"들 사이에서만 랜덤. 초반엔 비슷한 수가 많아 다양해지고, 후반 전술 국면은 최선 수의 점수 격차가 커서 풀에 하나만 남아 **정밀도 유지**.
- **안전장치**: 즉시 승리·VCF 강제승·즉시 차단은 랜덤 이전에 처리(격차 100000+)되므로 절대 랜덤되지 않는다.
- 랜덤 선택을 위해 후보 점수는 **정확값**이어야 한다: 루트를 **전체 창(full window)**으로 평가한다(`minimaxMove`는 이미 그러함; `minimaxMoveTT`는 margin>0일 때 최종 선택용 루트 패스를 전체 창으로).
- 게임 플레이는 `Math.random` 사용(재현 불필요). Arena 검증 시엔 랜덤이 판을 다양화하므로 별도 오프닝 랜덤 없이도 표본이 갈린다.

**초반만 랜덤 (강함 보존)**: 깊은 탐색(어려움/d6)은 평가가 세밀해 "100점 차이"도 진짜 실수라, **매 수 랜덤하면 실수가 누적돼 약해진다**. 그래서 `MinimaxPlayer`에 `randomUntilStones`를 두어 **판의 돌 수가 이 값 이하일 때(=초반)만 랜덤**하고, 이후는 결정적(최선)으로 둔다. 초반 몇 수만 갈라져 **모양은 다양해지고 실력은 거의 유지**(강한 엔진의 오프닝 다양화와 동일 원리).

**적용 & 검증 결과(2026-07-11, 채택됨)**:
- `minimaxMove(..., margin)`, `minimaxMoveTT(..., margin)`.
- **보통(d2): `margin 100`(전 구간)** — 얕은 평가라 전 구간 랜덤해도 무손실. Arena: 랜덤 vs 결정 **0.53**(강함 유지), 다양성 10종.
- **어려움(d6): `margin 600` + `randomUntilStones 12`(초반만)** — 전 구간 랜덤은 0.15~0.25로 약해졌으나, 초반만 랜덤 시 **0.46**(≈0.5, 강함 유지) + 다양성 8종. → 채택.
- **불변식**: 즉시승·VCF·즉시차단은 랜덤 이전 처리(격차 100000+) → 절대 랜덤되지 않음.
