...
### 범위 밖
- 오프닝(타라구치-10) 절차. **Arena는 오프닝 없이(`useOpening=false`) 자유 착수로 대국**한다. 오프닝 스왑까지 포함한 대전은 향후 확장(§10)으로 남긴다.
...
| AC-A15 | 기존 테스트 전부 통과(회귀 없음) |

---

## 10. 향후 확장 (Out of scope, 기록만)

- 오프닝(타라구치-10) 포함 대전 — `AiPlayer`의 오프닝 메서드 6종을 사용해 스왑까지 자동화.
